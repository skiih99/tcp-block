...
void forward_sendpkt(pcap_t* handle, uint8_t* mac, uint8_t* org_packet, struct ethhdr* org_eth, struct iphdr* org_ip, struct tcphdr* org_tcp) {
    ...
    sendtcp->checksum = 0;
    uint16_t temp2 = htons(checksum_tcp(*sendtcp, *sendip, sizeof(struct tcphdr), org_datalen));
    ...
    pcap_sendpacket(handle, packet, ethhdr_size + org_iplen + org_tcplen);
